...
// @ts-ignore - Não existem types para meyda
import * as Meyda from 'meyda';
// @ts-ignore - Não existem types para node-wav
import * as wav from 'node-wav';

import { MFCC, X, Y, Row } from './types';
import { bufferSize, labels, testSplit, xSize, xStep, dataBasePath } from './constants';
...
const getRows = () => {
    ...
    labels.forEach(label => {
        const fileNames = fs
            .readdirSync(`${dataBasePath}/raw/${label}`)
        ...
        fileNames.forEach((fileName, i) => {
            // Reads buffers
            const buffer = fs.readFileSync(`${dataBasePath}/raw/${label}/${fileName}`);
            ...
            console.log(
                `Preprocessing (${i}/${fileNames.length}) ${label}/${fileName}. Added ${examplesCount} examples.`,
            );
        });
    });
...
// Save mfccs to file
fs.writeJsonSync(`${dataBasePath}/data.json`, getData());
